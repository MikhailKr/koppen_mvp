...
st.set_page_config(page_title="Create Wind Farm - Koppen", page_icon="⚡", layout="wide")
init_session_state()
inject_css()
render_sidebar()
require_auth()
...
if current_step == 1:
    ...
    with st.form("farm_form"):
        farm_name = st.text_input("Farm Name *", placeholder="e.g., North Sea Wind Park")
...
# ==================== STEP 2: Add Turbines ====================
    ...
    if current_fleets:
        ...
        for fleet in current_fleets:
            ...
            with st.container(border=True):
                ...
                with col2:
                    st.markdown(f"📍 ({loc.get('latitude', 0):.2f}, {loc.get('longitude', 0):.2f})")
                with col3:
    ...
    with tab_existing:
        if not all_turbines:
            st.info("No turbine specs available. Create one in the 'Create New Turbine' tab.")
        else:
            turb_opts = {f"{t.get('turbine_type', 'Unknown')} ({t['nominal_power']}MW)": t['id'] for t in all_turbines}
            selected_turb = st.selectbox("Select Turbine", list(turb_opts.keys()), key="existing_turb")
    ...
    with tab_new:
        ...
        new_turb_type = st.text_input("Turbine Model", placeholder="e.g., Vestas V150", key="new_turb_type")
        col1, col2 = st.columns(2)
        with col1:
            new_hub_height = st.number_input("Hub Height (m)", value=100.0, min_value=10.0, max_value=300.0, key="new_hub")
        with col2:
            new_nominal_power = st.number_input("Nominal Power (MW)", value=4.0, min_value=0.1, max_value=20.0, key="new_power")
        
        # Power curve selection or creation
        st.markdown("**Power Curve:**")
        curve_choice = st.radio("", ["Use Existing", "Create New"], horizontal=True, key="curve_choice")
        ...
        if curve_choice == "Use Existing":
            if all_curves:
                ...
                selected_curve = st.selectbox("Select Power Curve", list(curve_opts.keys()), key="existing_curve")
                new_curve_id = curve_opts[selected_curve]
                
                # Show selected curve preview
                selected_curve_data = next((c for c in all_curves if c['id'] == new_curve_id), None)
                if selected_curve_data and selected_curve_data.get('wind_speed_value_map'):
                    ...
                    st.line_chart(chart_data, x="Wind Speed (m/s)", y="Power (kW)", height=200)
            else:
            ...
            if curve_method == "Auto-generate (scaled)":
                ...
                chart_data = pd.DataFrame([
                    {"Wind Speed (m/s)": float(k), "Power (kW)": v}
                    for k, v in sorted(default_curve.items(), key=lambda x: float(x[0]))
                ])
                st.line_chart(chart_data, x="Wind Speed (m/s)", y="Power (kW)", height=200)
                ...
                if "manual_curve_points" not in st.session_state or len(st.session_state.manual_curve_points) < 10:
                ...
                with btn_col1:
                    if st.button("🔄 Sort & Refresh", key="sort_points", use_container_width=True, type="primary"):
                        st.session_state.manual_curve_points.sort(key=lambda x: x["wind_speed"])
                        st.rerun()
                with btn_col2:
                    st.markdown(f"**{len(st.session_state.manual_curve_points)} points**")
                ...
                for i, p in enumerate(st.session_state.manual_curve_points):
                    table_html += f'<tr style="background: #1e293b; color: #e2e8f0;">'
                ...
                with add_cols[0]:
                    add_ws = st.number_input("Wind Speed (m/s)", value=0.0, min_value=0.0, max_value=50.0, step=0.5, key="add_ws")
                with add_cols[1]:
                    add_pwr = st.number_input("Power (kW)", value=0, min_value=0, max_value=50000, step=10, key="add_pwr")
                with add_cols[2]:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("➕ Add", key="add_point", type="primary"):
                        st.session_state.manual_curve_points.append({"wind_speed": add_ws, "power": add_pwr})
                        st.session_state.manual_curve_points.sort(key=lambda x: x["wind_speed"])
                        st.rerun()
                ...
                with edit_cols[0]:
                    edit_idx = st.number_input("Point #", min_value=1, max_value=len(st.session_state.manual_curve_points), value=1, key="edit_idx")
                with edit_cols[1]:
                    current_point = st.session_state.manual_curve_points[edit_idx - 1]
                    new_ws = st.number_input("Wind Speed", value=float(current_point["wind_speed"]), min_value=0.0, max_value=50.0, step=0.5, key="edit_ws")
                with edit_cols[2]:
                    new_pwr = st.number_input("Power (kW)", value=int(current_point["power"]), min_value=0, max_value=50000, step=10, key="edit_pwr")
                with edit_cols[3]:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("✏️ Update", key="update_point"):
                        st.session_state.manual_curve_points[edit_idx - 1] = {"wind_speed": new_ws, "power": new_pwr}
                        st.rerun()
                
                # Delete section
                del_cols = st.columns([1, 2, 1])
                with del_cols[0]:
                    del_idx = st.number_input("Delete #", min_value=1, max_value=len(st.session_state.manual_curve_points), value=1, key="del_idx")
                with del_cols[1]:
                    if len(st.session_state.manual_curve_points) > 3:
                        if st.button("🗑️ Delete Point", key="delete_point"):
                ...
                if len(default_curve) >= 2:
                    ...
                    st.line_chart(chart_data, x="Wind Speed (m/s)", y="Power (kW)", height=180)
                    
                    max_power = max(default_curve.values())
                    st.caption(f"Max Power: {max_power:,} kW | {len(default_curve)} points")
        
        if st.button("Create Turbine", key="create_new_turb", type="secondary"):
            # Create power curve if needed
            if curve_choice == "Create New":
                curve_result = api.create_power_curve(f"{new_turb_type} Curve", default_curve)
                if curve_result:
    ...
    loc_tab_existing, loc_tab_new = st.tabs(["Use Existing Location", "Create New Location"])
    
    with loc_tab_existing:
        if not all_locations:
            st.info("No locations available. Create one in the 'Create New Location' tab.")
            selected_loc_id = None
        else:
            loc_opts = {f"({l['latitude']:.2f}, {l['longitude']:.2f})": l['id'] for l in all_locations}
            selected_loc = st.selectbox("Select Location", list(loc_opts.keys()), key="existing_loc")
            selected_loc_id = loc_opts[selected_loc] if selected_loc else None
    
    with loc_tab_new:
        col1, col2 = st.columns(2)
        with col1:
            new_lat = st.number_input("Latitude", value=52.0, min_value=-90.0, max_value=90.0, key="new_lat")
        with col2:
            new_lon = st.number_input("Longitude", value=4.0, min_value=-180.0, max_value=180.0, key="new_lon")
        
        st.markdown("**Quick Add:**")
        cols = st.columns(3)
        for i, loc in enumerate(PREDEFINED_LOCATIONS[:6]):
            with cols[i % 3]:
                if st.button(f"📍 {loc['name']}", key=f"quick_loc_{loc['name']}", use_container_width=True):
                    result = api.create_location(latitude=loc["latitude"], longitude=loc["longitude"])
                    if result:
    ...
    num_turbines = st.number_input("How many turbines at this location?", min_value=1, max_value=500, value=10, key="num_turbs")
...
# ==================== STEP 3: Review & Finish ====================
    ...
    with col3:
        st.metric("Locations", len(set(f.get('location', {}).get('id') for f in fleets if f.get('location'))))
    ...
    for fleet in fleets:
        ...
        with st.container(border=True):
            ...
            with col1:
                st.markdown(f"**{fleet['number_of_turbines']}x {turb.get('turbine_type', 'Unknown')}**")
                st.caption(f"{turb.get('nominal_power', 0)} MW × {fleet['number_of_turbines']} = {turb.get('nominal_power', 0) * fleet['number_of_turbines']:.1f} MW")
            with col2:
                st.markdown(f"📍 ({loc.get('latitude', 0):.2f}, {loc.get('longitude', 0):.2f})")
    ...
    with col2:
        if st.button("➕ Create Another Farm", use_container_width=True):
            ...
            st.rerun()
    with col3:
        if st.button("✅ Finish & View Farms", use_container_width=True, type="primary"):
            st.session_state.wizard_step = 1
            st.session_state.wizard_farm = None
