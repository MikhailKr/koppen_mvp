    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            history = [
                ...
                for m in st.session_state.chat_messages[:-1]  # Exclude last message (current prompt)
            ]
...
if not st.session_state.chat_messages:
    st.info("👋 Hi! I'm your AI assistant. Ask me anything about your wind farms, forecasts, or generation data.")
    ...
    with col1:
        if st.button("📋 List my wind farms", use_container_width=True):
            st.session_state.chat_messages.append({"role": "user", "content": "What wind farms do I have?"})
            st.rerun()
    
    with col2:
        if st.button("📊 Check forecast accuracy", use_container_width=True):
            st.session_state.chat_messages.append({"role": "user", "content": "Show me the forecast accuracy for all my wind farms"})
            st.rerun()
    
    with col3:
        if st.button("⚡ Generation summary", use_container_width=True):
            st.session_state.chat_messages.append({"role": "user", "content": "Give me a summary of generation for my wind farms"})
            st.rerun()
