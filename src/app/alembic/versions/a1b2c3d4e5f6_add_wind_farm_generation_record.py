...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_windfarmgenerationrecord_timestamp'), 'windfarmgenerationrecord', ['timestamp'], unique=False)
    op.create_index(op.f('ix_windfarmgenerationrecord_wind_farm_id'), 'windfarmgenerationrecord', ['wind_farm_id'], unique=False)

    # Make wind_turbine_id nullable in windturbinegenerationrecord
    op.alter_column('windturbinegenerationrecord', 'wind_turbine_id',
                    existing_type=sa.INTEGER(),
                    nullable=True)

    # Add is_synthetic column to windturbinegenerationrecord
    op.add_column('windturbinegenerationrecord',
                  sa.Column('is_synthetic', sa.Boolean(), nullable=False, server_default='false'))
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_windfarmgenerationrecord_wind_farm_id'), table_name='windfarmgenerationrecord')
    op.drop_index(op.f('ix_windfarmgenerationrecord_timestamp'), table_name='windfarmgenerationrecord')
